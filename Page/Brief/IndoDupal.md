...
一直以来，作为地幔不均一力证的Dupal异常，其范围都没有确定下来。而印度洋作为最初发现Dupal特征的地方，显然是最适合圈定其范围的研究区。所以本文主要通过对印度洋洋中脊及洋壳部分的铅同位素数据进行分析处理，确定了Dupal异常的边界点，从而限制出Dupal特征的异常范围。在此基础上，通过前人对LLSVP，地幔柱的研究，探讨了Dupal与其影响因素的关系。最终确定印度洋Dupal异常区主要与核幔边界和被印度板块岩石圈污染的地幔两者有关，并对应印度洋Dupal特征区的两类特征区域。
...
因此，本文从发现Dupal异常的印度洋入手，通过已有数据的分析处理，在厘定印度洋Dupal异常范围后，力求对Dupal异常及背后意义做出合理的解释。
...
想要弄清楚印度洋Dupal区离轴的三部分与洋中脊的关系，或许我们得从其他方面入手。首先得考虑地质年龄的问题，将印度洋Dupal区放到磁条带年龄底图上得到图6。
...
目前海洋钻探的水平就注定我们能取得的玄武岩样品基本都是“表面”的，所以磁条带底图基本上就代表了样品数据点的年龄。显然，印度洋离轴的三部分正好对应着地质年龄更高的部分。而由前面U-Th-Pb同位素体系与Dupal特征的关系，我们可以发现在大洋扩张的背景下，离轴部分的Dupal特征有变高的趋势。但这里面又涉及到影响Dupal分布的其他潜在因素。
...
印度洋区域有不少地幔岩浆上升至地表形成的热点，其中最引人注目的是马里昂（Marion）、凯尔盖朗（Kerguelen）和留尼汪（Réunion）热点。刚好凯尔盖朗和留尼汪热点两者都在离轴高Dupal特征区范围，故需考虑热点在Dupal形成中的影响。目前已有不少人联系起壳幔边界的特征区域和Dupal特征区的关系。

就比如 **温联星（2006）** 认为南大西洋到印度洋的地幔底有VLVP（A rapidly structurally-varying very-low velocity province）区域存在，以运动的方式判断出典型Dupal特征来自于VLVP边界上的三个热点。但是这不能合理的解释洋中脊的Dupal特征，以及为什么没有热点影响的区域也有Dupal特征。

**Doucet（2020）** 的文章中则通过筛选出在两个大型LLSVP（large low shear-wave velocity province）上的深地幔柱源的Dupal数据点，从而得出非洲区域有不同于太平洋区域的俯冲型大陆物质富集，结合泛大陆的构造运动，一定程度上给出了Dupal特征的来源说明。在 **White（2015）** 的综述中，也认为Dupal与LLSVP存在一定关系。

当然，还有许多其他人的想法，但大致与上诉观点类似。他们对Dupal特征的来源做了限制，也就避免了其他存在的Dupal区域无法解释的问题。甚至他们间所使用的Dupal区域都有所不同，这也是一直没有统一的Dupal指标的遗留问题。

我认为，前人的观点里，热点、地幔柱、还有核幔边界的特征都能解释他们对应的Dupal特征来源，而本文划定的Dupal特征区，特别是洋中脊的边界区，明显要大于他们的范围。特别是西北印度洋中脊部分，都是VLVP和LLSVP没有涉及的区域。由前面的判断，中印度洋脊和卡尔斯伯格脊是存在典型的Dupal特征的。

**Tsekhmistrenko（2021）** 通过地震数据，判断出地幔柱并不是垂直的管道，而更像是树状结构。这个发现对温联星和Doucet对联系Dupal与地幔边界的想法有了一定的冲击，两者的范围类似并不一定能上下对应。但反过来，这对如何解释本文中的Dupal区域有直接的帮助。因为树状的结构，那么在地幔岩浆中的Dupal特征可能在上涌中分散开来，从而变成现在的沿轴和离轴两个方向上的分布不均。

总结来说，具有Dupal特征的岩浆有源于LLSVP和VLVP边界的成分。但洋中脊及不受热点影响的区域中的Dupal特征情况说明，还有其他的影响因素存在，还需要进一步考虑其他因素的影响。

#### 冈瓦纳大陆与Dupal特征的因果关系

**Dupré、Allègre、Hart** 都认同的Dupal异常有地幔受到污染的因素在。**Mazzucchelli（2016）** 在研究南美洲时也认为Dupal异常有下地壳部分的影响。
**Kumar（2007）** 指出印度构造板块的快速漂移造就了现今印度洋的模样。印度板块的岩石圈是冈瓦纳大陆（Gondwanaland）中最薄的，其岩石圈底部因快速移动熔融的部分自然也会进入到地幔。

本文划定的印度洋Dupal特征区，刚好就是冈瓦纳大陆裂解的中心部分，且大部分都是冈瓦纳大陆裂解后形成的洋壳部分。结合印度板块岩石圈成分进入地幔的情况来看，洋中脊上的Dupal特征自然会受到其影响。图中也可以明显的发现，东经90度海岭的数据点基本都有Dupal特征。
同理，可以解释亚丁湾脊没有Dupal特征，自然是其地幔并无类似印度板块岩石圈的成分影响。

## 结论

通过对印度洋洋中脊及离轴部分铅同位素数据的处理，可以圈定北纬10°，东经25°，东经125°三个边界点，并划定印度洋Dupal范围。限于西南印度洋洋中脊东经25°的数据密度较低，其边界范围可能还可以有所变动。整体的范围基本如图5所示。

参考前人对印度洋区域的热点、LLSVP与Dupal特征的关系的总结，结合目前发现的地幔柱树状结构与印度板块的快速漂移历史，在此推断印度洋的Dupal特征源于核幔边界和混合在地幔中印度洋岩石圈成分。两种来源则对应图6中两种Dupal特征区。
